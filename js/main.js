...
// отладочный хендл для автотестов (target/breakTarget/placeTarget повторяют логику кликов мыши)
const _dbgDir = new THREE.Vector3();
window.__game = {
  scene, camera, renderer, player, voxels,
  get ready() { return !!window.__sceneReady; },
  target() {
    camera.getWorldDirection(_dbgDir);
    return raycastVoxel(camera.position, _dbgDir, REACH);
  },
  breakTarget() {
    const hit = this.target();
    if (!hit || hit.y === 0) return null;
    voxels.delete(key(hit.x, hit.y, hit.z));
    rebuildChunks();
    return hit;
  },
  placeTarget() {
    ...
    if (!hit) return null;
    const px = hit.x + hit.nx, py = hit.y + hit.ny, pz = hit.z + hit.nz;
    if (getBlock(px, py, pz) || playerIntersectsBlock(px, py, pz)) return null;
    voxels.set(key(px, py, pz), HOTBAR[selected]);
    rebuildChunks();
